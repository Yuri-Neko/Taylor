    ...
    try {
        ...
        if (/use|pakai/i.test(command)) {
            try {
                ...
                } else if (args.length > 2 && args[0] === !'potion') conn.reply(m.chat, pickRandom(['Hanya bisa menggunakan potion', 'Mau ngunain apa? Cuma bisa gunain potion :v', 'Wih mau gunain apa kamu, kan hanya bisa potion', 'Waduheck, hanya bisa potion', 'lah, mau gunain apa?, kan hanya bisa potion']) + '\nContoh penggunaan: *' + usedPrefix + 'use potion 1*', m)
            } catch (e) {
                console.log(e)
                throw msgerror
            }
