...
conn.optionsData = conn.optionsData ? conn.optionsData : {};
  ...
  if (args[0] && !isNaN(args[0])) {
    ...
    conn.optionsData[m.chat] = { idop, key, timeout: setTimeout(() => { conn.sendMessage(m.chat, { delete: key }); delete conn.optionsData[m.chat]; }, 60 * 1000) };
  }
}
...
conn.optionsData = conn.optionsData ? conn.optionsData : {};
  ...
  const { timeout, idop, key } = conn.optionsData[m.chat];
